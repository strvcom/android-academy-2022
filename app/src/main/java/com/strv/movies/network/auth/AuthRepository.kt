...
class AuthRepository @Inject constructor(
    ...
    suspend fun logIn(username: String, password: String): Either<AuthError, Boolean> {
        return try {
            ...
            if (requestToken.isSuccess.not()) {
                Timber.d("Auth Error - Failed to get request token")
            ...
            if (validationResponse.isSuccess.not()) {
                Timber.d("Auth Error - Failed to validate credentials")
                return Either.Error(AuthError.INVALID_CREDENTIALS)
            }
            val createSessionResponse =
                authApi.createSession(CreateSessionBody(validationResponse.requestToken))
            if (createSessionResponse.isSuccess.not()) {
                Timber.d("Auth Error - Failed to create new session")
                return Either.Error(AuthError.NETWORK_ERROR)
            }
            ...
            Timber.d("Auth Error - Some network fail - ${t.localizedMessage}")
    ...
    suspend fun logOut(): Either<String, Unit> {
        return try {
            val sessionToken =
                authDataStore.sessionToken ?: return Either.Error("User not authenticated")
